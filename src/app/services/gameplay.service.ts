...
export class GameplayService {
    ...
    constructor(private soundService: SoundService) {
        ...
        this.loadingState = 0;
        this.shipOrientations = ["vertical", "horizontal"];
    }

    init(): Promise<any> {
        this.rendererAlly = PIXI.autoDetectRenderer(320, 576, {
            antialias: true,
            backgroundColor: 0x003454
        });
        this.rendererEnemy = PIXI.autoDetectRenderer(320, 576, {
            antialias: true,
            backgroundColor: 0x003454
        });
        ...
        function runGame(res?: PIXI.loaders.Loader, loader?: any) {
            ...
            self.shoreMapFlipped = new Tilemap(
                self.res.resources["shoremapflipped"].data
            );
        ...
        return new Promise((resolve, reject) => {
            PIXI.loader
                ...
                .add(
                    "shoremaptiled",
                    "/assets/sheets/tile_shore/gallery-metal-tile-kit-02.png"
                )
                .add("shoremap", "/assets/sheets/tile_shore/tile_shore.json")
                .add(
                    "shoremapflipped",
                    "/assets/sheets/tile_shore/tile_shore_flipped.json"
                )
                ...
                .add(
                    "ui_sheet",
                    "/assets/sheets/effects/effects_spritesheet.png"
                )
                .add("ui_atlas", "/assets/sheets/effects/effects.json")
                .load(function(res, loader) {
                    runGame(res, loader);
                    resolve();
                })
                .on("progress", loader => {
    ...
    drawBackground() {
        ...
        this.backgroundContAlly.addChild(
            this.createSeaBackground(waterTextures)
        );
        this.rootContainerAlly.addChildAt(this.backgroundContAlly, 0);

        this.backgroungContEnemy.addChild(
            this.createSeaBackground(waterTextures)
        );
    ...
    drawCtrls() {
        let topCntrlsAlly = this.createTilingSprite(
            "shoremaptiled",
            this.shoreMapFlipped.getMapData()
        );
        topCntrlsAlly.position.set(0, 0);
        let bottomCntrAlly = this.createTilingSprite(
            "shoremaptiled",
            this.shoreMap.getMapData()
        );
        this.topCtrlsContainerAlly.addChild(topCntrlsAlly);
        this.botCtrlsContainerAlly.addChild(bottomCntrAlly);

        let topCntrlsEnemy = this.createTilingSprite(
            "shoremaptiled",
            this.shoreMapFlipped.getMapData()
        );
        topCntrlsEnemy.position.set(0, 0);
        let bottomCntrEnemy = this.createTilingSprite(
            "shoremaptiled",
            this.shoreMap.getMapData()
        );
    ...
    private landShip(targetCell: any, arrOfPlacement: any, arrOfTouched: any) {
        this.selectedShip.currentSprite.parent.removeChild(
            this.selectedShip.currentSprite
        );
        ...
        if (this.selectedShip.orientation === "vertical") {
            ...
            x =
                landingCell.x +
                this.selectedShip.currentSprite.width *
                    this.selectedShip.currentSprite.anchor.x;
            y =
                landingCell.y +
                this.selectedShip.currentSprite.height *
                    this.selectedShip.currentSprite.anchor.y;
        } else {
            landingCell = this.gridMapAlly.getKey(arrOfPlacement[0]);
            x =
                landingCell.x +
    ...
    private getRandomPlacement(
        shipLength: number,
        shipRandomOrientation: string
    ) {
        //debugger;
    ...
    private getFreeSlotsListHorizonal(slotSize: number) {
        ...
        for (let i = 65; i < 65 + this.gridMapAlly.square - slotSize + 1; i++) {
            ...
            for (let j = 1; j < 1 + this.gridMapAlly.square; j++) {
                ...
                let cells = this.gridMapAlly.getSiblingsRight(
                    candidateKey,
                    slotSize - 1
                );
                cells.unshift(candidate);
                let touchedCells = this.gridMapAlly.getTouchedCells(
                    cells,
                    "horizontal",
                    false,
                    true
                );
                let cellsFree = cells.every(
                    cell => !cell.occupied && !cell.touched
                );
                let touchedCellsFree = touchedCells.every(
                    cell => !cell.occupied
                );
    ...
    private getFreeSlotsListVertical(slotSize: number) {
        ...
        for (let i = 1; i < 1 + this.gridMapAlly.square - slotSize + 1; i++) {
            ...
            for (let j = 65; j < 65 + this.gridMapAlly.square; j++) {
                ...
                let candidateKey = this.gridMapAlly.getByString(column, row);
                let candidate = this.gridMapAlly.get(candidateKey);
                let cells = this.gridMapAlly.getSiblingsBottom(
                    candidateKey,
                    slotSize - 1
                );
                cells.unshift(candidate);
    ...
    public randomLanding() {
        if (this.gameState === "landing")
            this.shipFabric.ships.forEach(ship => {
                if (!ship.landed) {
                    let shipRandomOrientation = this.shipOrientations[
                        Math.round(Math.random())
                    ];
                    let variations = this.getRandomPlacement(
                        ship.length,
                        shipRandomOrientation
                    );
                    let randomPlace =
                        variations[this.getRandomInt(0, variations.length - 1)];
                    this.selectedShip = ship;
                    this.selectedShip.changeOrientation(shipRandomOrientation);
                    this.landShip(
                        randomPlace.cells[0],
                        randomPlace.cells,
                        randomPlace.touchedCells
                    );
                }
            });
    }
    ...
    private createEnemyBurningSpot() {
        let burningSpot = new ComplexEffect("spot.png", "burn_${i}.png", 32, 1);
        return burningSpot;
    }

    onEnemyFired(targetData: GameEvent) {
        let targetCellGraphic = this.gridMapAlly.getByString(
            targetData.column,
            targetData.row
        );
    ...
    onPlayerFireResults(data: GameEvent) {
        let targetCellGraphic = this.gridMapEnemy.getByString(
            data.column,
            data.row
        );
    ...
    private onPlayerMissed(targetCellGraphic: PIXI.Graphics, targetCell: any) {
        ...
        this.gridContainerEnemy.setChildIndex(
            this.explosionEffectEnemy.sprite,
            this.gridContainerEnemy.children.length - 1
        );
        this.explosionEffectEnemy.sprite.onFrameChange = () => {
            if (
                !sign.sprite.visible &&
                this.explosionEffectEnemy.sprite.currentFrame > 12
            ) {
    ...
    private onPlayerKill(
        targetCellGraphic: PIXI.Graphics,
        targetCell: any,
        killedCells: any,
        killedOrientation: string
    ) {
        let i;
        let deadShip: PIXI.Sprite = this.shipFabric.createDeadShipSprite(
            killedCells.length,
            killedOrientation
        );
        for (i = 0; i < killedCells.length; i++) {
            ...
            let cellGraphic: PIXI.Graphics = this.gridMapEnemy.getByString(
                coords[0],
                coords[1]
            );
            ...
            explosion.sprite.onFrameChange = () => {
                if (cell.effect && explosion.sprite.currentFrame > 12) {
                    cell.effect.container.parent.removeChild(
                        cell.effect.container
                    );
                ...
                if (
                    killedCells.length === 1 &&
                    explosion.sprite.currentFrame > 12
                ) {
                    deadShip.visible = true;
                    this.actionsAllowed = true;
                }
    ...
    private onPlayerHit(targetCellGraphic: PIXI.Graphics, targetCell: any) {
        ...
        this.explosionEffectEnemy.sprite.onFrameChange = () => {
            if (
                ...
                spot.effect.sprite.position.set(
                    0,
                    -Math.floor(spot.effect.sprite.height * 0.3)
                );
    ...
    private onEnemyMissed(targetCellGraphic: PIXI.Graphics, targetCell: any) {
        ...
        this.gridContainerAlly.setChildIndex(
            this.explosionEffectAlly.sprite,
            this.gridContainerAlly.children.length - 1
        );
        this.explosionEffectAlly.sprite.onFrameChange = () => {
            if (
                !sign.sprite.visible &&
                this.explosionEffectAlly.sprite.currentFrame > 12
            ) {
                sign.sprite.position.set(
                    targetCellGraphic.x + targetCellGraphic.width * 0.5,
                    targetCellGraphic.y + targetCellGraphic.height * 0.5
    ...
    private onEnemyKill(
        ...
        for (i = 0; i < killedCells.length; i++) {
            let cellCoords = killedCells[i];
            let coords = [cellCoords[0], cellCoords.slice(1)];
            let cellGraphic: PIXI.Graphics = this.gridMapAlly.getByString(
                coords[0],
                coords[1]
            );
    ...
    private createGrid(size: number) {
        ...
        let createCell = (
            ...
            cell.lineStyle(2, 0x333333, 0.1).drawRect(0, 0, width, height);
            //events on grid
            if (isEnemy) {
                cell.on("click", function(event) {
                    self.onEnemyGridCellClick(event, this);
                })
                    .on("tap", function(event) {
                        self.onEnemyGridCellClick(event, this);
                    })
                    .on("mouseover", function(event) {
                        self.onEnemyGridCellMouseEnter(event, this);
                    });
                // .on('mouseout', function(event) { self.onGridCellMouseLeave(event, this)})
            } else {
                cell.on("click", function(event) {
                    self.onGridCellClick(event, this);
                })
                    .on("tap", function(event) {
                        self.onGridCellClick(event, this);
                    })
                    .on("mouseover", function(event) {
                        self.onGridCellMouseEnter(event, this);
                    });
                // .on('mouseout', function(event) { self.onGridCellMouseLeave(event, this)})
            }
        ...
        this.gridContainerAlly.on("mouseout", function(event) {
            self.onGridCellMouseLeave(event, this);
        });
    }
    ...
    addSeletionShips() {
        ...
        let stackOfShips = this.shipFabric.createDefaultStackOfShips(
            "vertical"
        );
        let i;
        for (i = 0; i < stackOfShips.length; i++) {
            ...
            ship.setPosition(
                (totalWidth / stackOfShips.length) * i +
                    ship.currentSprite.width / 2,
                64,
                true
            );
            ship.currentSprite.interactive = true;
            ship.currentSprite.on("click", event => onShipClick(event, ship));
        ...
        let selectShip = (ship: Ship) => {
            if (this.selectedShip) {
                ...
                this.selectShipsContainer.addChild(
                    this.selectedShip.currentSprite
                );
            ...
            ship.currentData.position.set(
                this.topCtrlsContainerAlly.width / 2,
                this.topCtrlsContainerAlly.height / 2
            );
            this.topCtrlsContainerAlly.addChild(ship.currentSprite);
            this.hoverFigure = new PIXI.Sprite(
                this.selectedShip.currentSprite.texture.clone()
            );
    ...
    onGridCellClick(event, target) {
        if (this.selectedShip && this.landingAllowed) {
            ...
            let arr;
            if (this.selectedShip.orientation === "vertical") {
                arr = this.gridMapAlly.getCentredVerticSlice(
                    target,
                    this.selectedShip.length
                );
            } else {
                arr = this.gridMapAlly.getCentredHorizontalSlice(
                    target,
                    this.selectedShip.length
                );
            }
            this.landShip(
                target,
                arr,
                this.gridMapAlly.getTouchedCells(
                    arr,
    ...
    onGridCellMouseEnter(event, target) {
        if (this.selectedShip) {
            ...
            this.drawPreview(
                arr,
                this.gridMapAlly.checkTouchedCells(
                    arr,
                    this.selectedShip.orientation
                )
            );
    ...
    onEnemyGridCellClick(event, target) {
        if (
            ...
            this.targetElement.sprite.position.set(
