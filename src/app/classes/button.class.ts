...
export class Button implements Animatable {
    ...
    constructor(
        ...
        this.sprite.pivot.set(
            this.size["pivot_x"] || Math.ceil(this.sprite.width / 2),
            this.size["pivot_y"] || Math.ceil(this.sprite.height / 2)
        );
    ...
    private attachCallbacks(eventCallbacks: EventCallbacks): void {
        ...
        if (eventCallbacks.mouseover) {
            this.sprite.on("mouseover", event =>
                eventCallbacks.mouseover(event, this)
            );
        }
        if (eventCallbacks.mouseout) {
            this.sprite.on("mouseout", event =>
                eventCallbacks.mouseout(event, this)
            );
        }
